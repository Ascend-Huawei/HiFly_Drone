...
def validate_paths():
	...
	PROJECT_DIR = "/".join(SRC_PATH.split("/")[:-2])
	HOME_PATH = "/".join(SRC_PATH.split("/")[0:-6])
	SAMPLES_DIR = os.path.join(HOME_PATH, SRC_PATH.split("/")[-6])
	MODEL_PATH = os.path.join(PROJECT_DIR, "model")
	PRESENTER_SERVER_SH = os.path.join(SAMPLES_DIR, "common/run_presenter_server.sh")
	PRESENTER_SERVER_CONF = os.path.join(PROJECT_DIR, "src/uav_presenter_server.conf")
	paths = {
		"SRC_PATH": SRC_PATH,
		"HOME_PATH": HOME_PATH, 
